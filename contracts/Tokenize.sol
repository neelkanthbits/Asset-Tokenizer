/**
 * @file Tokenize.sol
 # @author https://github.com/amityadav0
 *
 * contract to create erc20 tokens.
 */
...
contract Tokenize is StandardToken, OwnerVerList {
    ...
    constructor(
        ...
        bool _privateVerificationList,
        address _privateListAddress,
        ...
        tokenURI = _tokenURI;
        privateVerificationList = _privateVerificationList;
        privateList = IVerificationList(_privateListAddress);
    ...
    function transfer(address _to, uint256 _value) public whenNotPaused {
        if (
            privateVerificationList == true && ownerVerificationList == true
        ) {
            // private Check
            require(
                !privateList.isBlackListed(msg.sender) &&
                    !privateList.isBlackListed(_to)
            );
            require(
                privateList.isWhiteListed(msg.sender) &&
                    privateList.isWhiteListed(_to)
            );
        ...
        } else if (privateVerificationList == true) {
            // private Check
            require(
                !privateList.isBlackListed(msg.sender) &&
                    !privateList.isBlackListed(_to)
