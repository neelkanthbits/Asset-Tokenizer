...
contract AssetTokenizationStorage {
    ...
    struct Token {
        ...
        bool privateVerificationList; // Should the token use private's verification list or the owners'
...
contract UpdatedAssetTokenization is
    Upgradeable,
    InitializableOwnable,
    AssetTokenizationStorage,
    AddedStorage,
    InitializeInterface
{
    ...
    function tokenizeAsset(
        ...
        Tokenize tokenized = new Tokenize(
            ...
            token.supply,
            token.tokenURI,
            token.privateVerificationList,
            verificationListAddress,
            token.ownerVerificationList,
            contractOwner
        );
        //If they want to use private verification list then whitleist the owner
        if (token.privateVerificationList == true)
